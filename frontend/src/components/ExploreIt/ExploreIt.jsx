    ...
    <div className='explore-t' id='explore-it'>
      ...
      <div className="explore-t-list">
        {it_list.map((item, index) => {
          return (
            <div
              onClick={() => setCategory(prev => prev === item.it_name ? "All" : item.it_name)}
              key={index}
              className='explore-t-list-item'
            >
              <img className={category === item.it_name ? "active" : ""} src={item.it_image} alt={item.it_name} />
              <p>{item.it_name}</p>
            </div>
          )
        })}
